...
static void read_cpu_load_sample(int cpu, struct cpu_load_sample *s)
{
 ...
 *
 * Uses delta sampling: kernel_cpustat counters are cumulative since
 * boot, so a raw idle/total ratio would barely move after the device
 * has been up for a while. We keep the previous sample per-CPU and
 * compute load from the DELTA between successive calls instead.
 * =================================================================== */

static struct cpu_load_sample prev_cpu_sample[NR_CPUS];
static bool prev_cpu_sample_valid[NR_CPUS];
...
static u64 get_cluster_load(int first_cpu, int num_cpus)
{
	...
	for (cpu = first_cpu; cpu < first_cpu + num_cpus; cpu++) {
		if (cpu_online(cpu)) {
			...
			read_cpu_load_sample(cpu, &s);

			if (prev_cpu_sample_valid[cpu] &&
			    s.total > prev_cpu_sample[cpu].total) {
				total_delta = s.total - prev_cpu_sample[cpu].total;
				idle_delta = (s.idle + s.iowait) -
					     (prev_cpu_sample[cpu].idle +
					      prev_cpu_sample[cpu].iowait);

				if (total_delta > 0) {
					total_load += 100 -
						div64_u64(idle_delta * 100, total_delta);
					counted++;
				}
			}

			prev_cpu_sample[cpu] = s;
			prev_cpu_sample_valid[cpu] = true;
		}
	}
