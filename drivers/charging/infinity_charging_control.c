/*
 * Infinity Charging Control Driver v1.0.35
 * SM8250-AC (Snapdragon 860) — Poco X3 Pro (vayu/bhima)
 *
 * Provides 4 charging modes via /sys/class/power_supply/battery/:
...
/* Charging mode: 0=bypass, 1=normal, 2=80%, 3=90% */
static int charging_mode = 1;
static DEFINE_MUTEX(mode_lock);

/* SMB2 charger register base (will be ioremap'd) */
...
/* Sysfs show: current charging mode */
...
/* IOCTL handler for charging control */
...
/* Probe: called when device tree matches "qcom,sm8250" */
static int icc_probe(struct platform_device *pdev)
{
    int ret;

    pr_info("%s: Probing Infinity Charging Control v1.0.35\n", DRIVER_NAME);
    ...
    pr_info("%s: Initialized — mode=%d\n", DRIVER_NAME, charging_mode);
    return 0;
...
MODULE_DESCRIPTION("Charging Control Driver for SM8250-AC (Poco X3 Pro)");
